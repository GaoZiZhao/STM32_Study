...
void led_task_func(void *argument)
{
	...
	led_operation_t led_value = LED_INITED_VALUE;
	...
	for(;;)
	{
		...
		if(0 != led_queue)
		{
			//Receive a message from the queue without blocking (0 ticks).
			if(pdTRUE == xQueueReceive(led_queue, &led_value, (TickType_t)0))
			{
				...
				if(LED_OK == led_ret)
				{
					printf("LED on off Success!\r\n");
				}
				else
				{
					printf("LED on off Failed!\r\n");
				}
			}
		}
		osDelay(100);
